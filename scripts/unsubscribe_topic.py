...
def usage():
    print("Usage: python unsubscribe_topic.py <node_letter> <topic>")
    print("Example: python unsubscribe_topic.py A news")
    sys.exit(1)
...
try:
    url = f"{node_api_url}/unsubscribe"
    payload = {"topic": topic}
    resp = requests.post(url, json=payload, timeout=5)
    print(f"Unsubscribe status: {resp.status_code} {resp.reason}")
    if resp.text.strip():
